...
from coding_agent import code_app
from deep_research_agent import research_app
from math_agent import math_app
from rag_tool import LocalLibrarian
from search_tool import WebScout

# --- CONFIGURATION ---
ORCHESTRATOR_MODEL = "qwen2.5-coder:14b" 
VISION_MODEL = "qwen2.5-vl:7b" # <--- NEW: Must be a vision-capable model
MAX_DEPTH = 3
...
def analyze_image(image_path: str, context_query: str) -> str:
    ...
    console.print(f"[Orchestrator] 👁️  Analyzing visual output: {image_path}...")
...
def call_deep_research(query: str) -> str:
    ...
    console.print(f"\n[Orchestrator] 📚 Delegating to Deep Research: '{query}'")
...
def call_math_lab(query: str) -> str:
    ...
    console.print(f"\n[Orchestrator] 🧮 Delegating to Math Lab: '{query}'")
...
def call_code_lab(query: str) -> str:
    ...
    console.print(f"\n[Orchestrator] 💻 Delegating to Code Lab: '{query}'")
...
def call_web_scout(query: str) -> str:
    console.print(f"\n[Orchestrator] 🌍 Scouting web for: '{query}'")
...
def planner_node(state: OrchestratorState):
    ...
    llm = ChatOllama(model=ORCHESTRATOR_MODEL, temperature=0.1)
...
def executor_node(state: OrchestratorState):
    ...
    if tool == "LIBRARIAN":
        ...
        console.print(f"\n[Orchestrator] 🔍 Reading files for: '{query}'")
