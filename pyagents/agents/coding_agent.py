...
from pyagents.config import CODING_MODEL_CONFIG, MAX_RETRIES, SCRIPT_NAME, TEST_NAME
from pyagents.utils import run_llm, run_vision_llm, extract_code, get_third_party_imports
...
def tester_node(state: SandboxState):
    ...
    with console.status("[cyan]Writing tests..."):
        response = run_llm(CODING_MODEL_CONFIG["tester"], prompt, system_prompt)
...
def coder_node(state: SandboxState):
    ...
    with console.status("[green]Generating solution..."):
        response = run_llm(CODING_MODEL_CONFIG["coder"], prompt, temperature=0.2)
...
def verifier_node(state: SandboxState):
    ...
    critique = run_vision_llm(CODING_MODEL_CONFIG["verifier"], prompt_text, image_path, temperature=0.1)
