...
# --- WARNING SUPPRESSION ---
# Silences the "package renamed" warning and unclosed socket warnings
...
class WebScout:
    def __init__(self):
        # We do NOT instantiate DDGS here anymore to avoid keeping the socket open.
    ...
    def search_metadata(self, queries: List[str]) -> List[Dict]:
        ...
        # FIX: Use Context Manager 'with' to ensure sockets close
        with DDGS() as ddgs:
            for q in queries:
                try:
                    # max_results is small to keep it fast
